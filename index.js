...
bot.on("message", async (msg) => {
  ...
  if (userStates[chatId]) {
    if (userStates[chatId].stage === "contact_shared") {
      const pin = msg.text;
    ...
    } else if (userStates[chatId].stage === "change_password") {
      ...
      if (/^\d{4}$/.test(newPassword)) {
        try {
          ...
          bot.sendMessage(
            chatId,
            "Password successfully changed. You can now log in with your new 4-digit PIN."
          );
          delete userStates[chatId]; // Clear user state after successful password change
        } catch (err) {
          bot.sendMessage(
            chatId,
...
// Event listener for /changepassword command
bot.onText(/\/changepassword/, async (msg) => {
  ...
  try {
    ...
    if (existingUser) {
      userStates[chatId] = { stage: "change_password" };
      bot.sendMessage(chatId, "Please enter your new 4-digit PIN:");
    } else {
      bot.sendMessage(
        chatId,
        "You are not signed up yet. Use /start to sign up."
      );
    }
  } catch (err) {
    bot.sendMessage(chatId, "An error occurred. Please try again later.");
